...
function animalClick(e){
  ...
  if(!isQuiz){
    ...
    if(e.target.eng === animals[currentAnimal].eng){
      ...
      sound.play();
		chinCharDiv.style.visibility = 'hidden';
		chinCharDiv.innerHTML = '<p>Good Job!</p>';
		chinCharDiv.style.visibility = 'visible';
...
function stopQuiz(){
  isQuiz = false;
  thumbsDiv.style.backgroundColor = '';
	bigPicDiv.style.backgroundColor = '';
  quizBtn.innerHTML = "Quiz Me!"
...
function notifyUser(guess){
  ...
  chinCharDiv.style.cssText += "justify-content: center; align-items: center; repeat: no-repeat;";
//		  var thumbColor = '';
//	  var bigPicColor = '';
...
function displayOutput(){
  ...
  // Percent correct
  let percent = (numCorrect / totalAttempts).toFixed(2) * 100;
